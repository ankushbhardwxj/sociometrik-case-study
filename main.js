...
timeSeries1.onAdd = function() {
  return L.DomUtil.get("timeseries1");
}
...
d3.csv("https://raw.githubusercontent.com/plotly/datasets/master/finance-charts-apple.csv", (rows, err) => {
  ...
  Plotly.newPlot("timeseries1", data, { title: "TimeSeries Graph 1" }, { scrollZoom: true });
});

const timeSeries2 = L.control({ position: "topright" });
timeSeries2.onAdd = function() {
  return L.DomUtil.get("timeseries2");
}
timeSeries2.addTo(map);
let arr2 = [];
d3.csv("https://raw.githubusercontent.com/plotly/datasets/master/finance-charts-apple.csv", (rows, err) => {
  arr2.push(rows);
}).then(() => {
  arr2 = arr2.slice(100, 150);
  const unpack = (rows, key) => {
    return rows.map(row => row[key]);
  }
  let trace1 = {
  ...
  let trace2 = {
    ...
    name: 'Type 2',
    x: unpack(arr2, 'Date'),
    y: unpack(arr2, 'AAPL.Low'),
    line: {color: 'blue'}
  }
  let data = [trace1, trace2];
  Plotly.newPlot("timeseries2", data, { title: "TimeSeries Graph 2" }, { scrollZoom: true });
})
